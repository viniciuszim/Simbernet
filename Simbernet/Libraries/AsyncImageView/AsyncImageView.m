...
{
	[[AsyncImageLoader sharedLoader] loadImageWithURL:imageURL target:self success:@selector(setImage:) failure:@selector(setNoImage:) ];
}
...
- (void)cancelLoadingURL:(NSURL*)url forTarget:(id)target
{
    [[AsyncImageLoader sharedLoader] cancelLoadingURL:url target:target];
}
...
{
    if (image && image != self.image && self.crossfadeDuration) {
    ...
    if (image == nil) {
        super.image = [UIImage imageNamed:@"defaultProductPhoto"];
    }
    
    //call
    if (self.finishLoadingBlock != nil) {
        self.finishLoadingBlock(image);
    }
}

- (void)setNoImage:(NSURL *)imageURL
{
    if (self.finishLoadingWithErrorBlock != nil) {
        self.finishLoadingWithErrorBlock();
    }
}
